...
GamePolicy.prototype.handleBallInHole = function(ball){
    ...
    // Matches Ball's pocket shrink animation duration so the ball is fully
    // hidden right as the animation finishes, not cut off mid-shrink.
    setTimeout(function(){ball.out();}, 260);
...
GamePolicy.prototype.updateTurnOutcome = function(){
    ...
    if(this.won){
        ...
        if(this.gameOverPending || !AI.finishedSession){
            return;
        }
        this.gameOverPending = true;

        let winnerIndex = this.foul ? (this.turn+1)%2 : this.turn;
        this.players[winnerIndex].totalScore.increment();

        // Give the final pocket sound and ball animation a beat to finish
        // before the game over screen takes over.
        setTimeout(function(){
            Game.showGameOver(winnerIndex);
        }, 1200);
