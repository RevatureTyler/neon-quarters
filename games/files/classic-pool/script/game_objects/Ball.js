...
Ball.prototype.update = function(delta){
    ...
    this.updatePocketAnimation();
}

// Smoothly shrinks a pocketed ball toward its resting point instead of it
// just sitting still for a beat and then vanishing outright.
Ball.prototype.updatePocketAnimation = function(){
    if(!this.pocketing)
        return;

    var POCKET_DURATION = 260;
    var elapsed = performance.now() - this.pocketStartTime;
    var t = Math.min(elapsed / POCKET_DURATION, 1);

    this.pocketScale = 1 - t;
...
Ball.prototype.updatePosition = function(delta){
	...
	if(Game.policy.isInsideHole(newPos)){
        ...
        this.pocketing = true;
        this.pocketScale = 1;
        this.pocketStartTime = performance.now();
...
Ball.prototype.draw = function () {
    ...
    var scale = this.pocketing ? this.pocketScale : 1;
	Canvas2D.drawImage(this.sprite, this.position, 0, scale, new Vector2(25,25));
};