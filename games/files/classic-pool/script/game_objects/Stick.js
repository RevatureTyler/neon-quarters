...
// Reflects the human player's current shot power in the on-screen meter, so
// charging a shot has clear visual feedback beyond the subtle stick pull-back.
function updatePowerMeter(power){
  var meter = document.getElementById('powerMeter');
  var fill = document.getElementById('powerMeterFill');
  if(!meter || !fill) return;

  if(power > 0){
    meter.classList.add('visible');
    fill.style.height = Math.min((power/75)*100, 100) + '%';
  }
  else{
