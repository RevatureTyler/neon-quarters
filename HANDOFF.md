...
- 29 games in `games/games.json`, each an `id` pointing at either an embedded HTML5 game (`games/files/<id>/index.html`, loaded in a same-origin iframe) or a Flash `.swf` (played via Ruffle).
- `js/player.js` is the core game-loading logic: builds the iframe, wires touch controls, and — the trickiest part — `setupResponsiveIframe()` auto-scales each game's fixed-size legacy page to fill the player frame via a measured `transform: scale()`. Games with their own responsive resize logic are excluded via `SELF_SIZING_GAMES` (currently `classic-pool`, `astray`, `bullet-hell`, `lights-out`, `genius`, `googol`, `jokenpo`, `bangbang`, `isocity` — the last five all size themselves off `window.innerWidth`/`innerHeight` at load rather than using a fixed canvas). **Gotcha specific to that last group:** none of them set an explicit page background color, relying on the browser's default white canvas. That default renders as transparent inside our black `.player-frame`, making black-on-white UI (checkboxes, line-art icons) go black-on-black and disappear. If a self-sizing game you're adding looks blank or half-invisible once embedded, check this first — fix is one `background: #fff` in its CSS, not a scaling issue.
- `js/games-data.js` has the shared `renderGameCard()` used across `site.js`/`player.js`/`leaderboards.js`.
- `js/leaderboards.js`, `js/achievements.js` — localStorage-driven (no backend): play counts, high scores, ratings, favorites, 10 achievement badges.
...
## Most recent work (this session, 2026-08-13)

Four commits, not yet pushed (local commits `ced3229`, `ed80d1a`, `94be72f`, plus this HANDOFF.md update, on top of the `55f789f` HEAD from the prior session):
...
**Not yet done, needs a human:** none of this has been pushed to `main` — these are local commits only. Given `main` is also the live GitHub Pages source with no build/review step in between, push was deliberately held back for the user to review first rather than auto-deploying overnight changes to a live site mid-AdSense-review.

## Prior session's scaling-bug fix (for context, already shipped)
...
- Browser caching required bumping the dev server's port repeatedly this session (9129 → 9131 → 9142 → 9143 → 9144 → 9145) — editing a JS/CSS file that's already been fetched once doesn't reliably bust the browser's cache on the same port, even against `python -m http.server`, which sends no cache-control headers of its own. Restarting on a fresh port is the reliable fix; killing the old one first (`pkill -f "http.server <port>"`) avoids leaking background processes. Reverted back to port 9129 (matching `.claude/launch.json`) before ending the session.
